...
- Markdownシンタックスハイライト、自動保存、Undo / Redo
- Idea Editorの現在の内容をローカルMarkdownファイルに保存
...
Windows 11ではpure_boardのアイコンが「隠れているインジケーター」に収納されることがあります。タスクバー右端の **^** からアイコンを表示領域へドラッグするか、Windowsの **設定 > 個人用設定 > タスクバー > その他のシステム トレイ アイコン** でpure_boardを有効にしてください。スキップ可能な初回チュートリアルでこの機能を紹介し、Settingsからは詳しい手順をいつでも確認できます。
...
- ローカル保存、自動起動、ネイティブ保存ダイアログ、ファイル操作用のTauriプラグイン
- CodeMirror 6
- React Markdown
